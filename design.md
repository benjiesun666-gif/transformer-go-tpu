...
- For each action, compute a “Bayesian score”:  
  `score = prior + uncertainty * exploration_weight * (1 - prior)`  
  where `exploration_weight` is a hyperparameter and `uncertainty` is the scalar output of the uncertainty head (broadcasted to all actions).
- The number of candidates to keep `k` is determined dynamically based on uncertainty:  
  `k = min_k + (max_k - min_k) * clip(uncertainty / threshold, 0, 1)`
...
- Checkpoints are saved using `orbax.checkpoint`, storing model parameters and optimizer state for resuming.
...
- **Support**: `torch` (only for data loading), `tensorboard`
